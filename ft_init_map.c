...
/*   Updated: 2025/01/27 18:20:06 by mohdahma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (y < 0 || y >= data->nb_lines || x < 0 || x >= data->length || data->p[y][x] == '1' || (data->p[y][x] == 'E' && i == 0))
        return ;
    if (i == 0)
    {
        if (data->p[y][x] == 'C')
    ...
    data->p[y][x] = '1';
    cheaking_path(data, y, x + 1, i);  // Move right
    cheaking_path(data, y + 1, x, i);  // Move down
    cheaking_path(data, y, x - 1, i);  // Move left
    cheaking_path(data, y - 1, x, i);  // Move up
}
